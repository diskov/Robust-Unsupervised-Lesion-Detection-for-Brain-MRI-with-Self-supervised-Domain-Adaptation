...
from uda_release.dset_classes.DsetSSQuadRand import DsetSSQuadRand
...
class BaurModel(nn.Module):
    ...
    def forward(self, x):
        mu, logvar = self.encode(x)
        z = reparameterize(mu, logvar)
        # z = mu  # for simple autoencoder to eliminate repameterization step and only use deterministic latent space mu
        y = self.decode(z)
...
class DCD(BasicModule):
    def __init__(self, h_features=128, input_features=128):
        ...
        self.fc2 = nn.Linear(h_features, 64)
...
def save_checkpoint(model, epoch, iteration, prefix=""):
    model_out_path = "./" + prefix + "model_epoch_{}_iter_{}.pth".format(epoch, iteration)
...
def estimate_log_p_x(model, x, mu, logvar, num_samples=5, x_sigma=1.0):
    ...
    # distributions
    q_z_x = dists.MultivariateNormal(mu, torch.diag_embed(sigma))
    p_z = dists.MultivariateNormal(torch.zeros_like(mu), torch.diag_embed(torch.ones_like(mu)))
...
def create_target_samples(hdf5_target_path, transform, n=39):
    ...
    # dataset = DsetSSQuadRand(brats_val_dataset)
...
def create_groups(X_s, Y_s, X_t, Y_t, seed=1):  # you can also add the mask if you want to have them as groups too
    """
    ...
    for i in range(4):
        for j in range(shot):
            G1.append((X_s[source_matrix[i][j * 1 - 1]], X_s[source_matrix[i][j * 1]]))
            Y1.append((Y_s[source_matrix[i][j * 1 - 1]], Y_s[source_matrix[i][j * 1]]))
...
# For beta anealing in Simple VAE training
...
def train_soft_intro_vae(dataset='cifar10', z_dim=128, lr_e=2e-4, lr_d=2e-4, batch_size=128, num_workers=4,
                         ...
                         num_epochs=250, num_vae=0, save_interval=50, recon_loss_type="l1",
    ...
    if dataset == 'cifar10':
        image_size = 32
        channels = [64, 128, 256]
        train_set = CIFAR10(root='./cifar10_ds', train=True, download=False, transform=transforms.ToTensor())
        ch = 3
    elif dataset == 'camcan':
        image_size = 128
        ch = 1
        ...
        image_size = 32
        channels = [64, 128, 256]
        train_set = SVHN(root='./svhn', split='train', transform=transforms.ToTensor(), download=False)
        ch = 3
    elif dataset == 'fmnist':
        image_size = 28
        channels = [64, 128]
    ...
    experiment_dir = './24_4_20220'
    os.makedirs(experiment_dir, exist_ok=True)

    # fig_dir = f'{experiment_dir}/Figures_masked_reconlosses_' + dataset + f"_beta_rec_{beta_rec}_beta_neg_{beta_neg}_beta_kl{beta_kl}_seed_{seed}"
    fig_dir = f'{experiment_dir}/Figures_simpleVAE' + dataset + f"_beta_rec_{beta_rec}__beta_kl{beta_kl}_annealing"
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if epoch % save_interval == 0 and epoch > 0:
            ...
            prefix = "24_4_20220/" + dataset + "_Simpple_VAE_beta_rec_" + str(beta_rec) + f"_beta_kl_{beta_kl}/"
            # prefix = "19_4_20220/" + dataset + "_bv3.5_maskedrec" + "_soft_intro" + "_betas_" + str(beta_kl) + "_" + str(beta_neg) + "_" + str(
            #     beta_rec) + f"_seed_{seed}/"
            save_checkpoint(model, save_epoch, cur_iter, prefix)
        ...
        for batch in pbar:
            ...
            if epoch < num_vae:
                ...
                # beta annealing
                beta_kl_annealing = monotonic_annealing(train_step=cur_iter, final_train_step=2975, beta_start=0.3, beta_final=0.0)

                # loss = loss_rec
                loss = beta_rec * loss_rec + beta_kl_annealing * loss_kl
                simple_loss_VAE.append(loss)
                decoder_loss_print.append(loss_rec)
                ...
                if cur_iter % test_iter == 0:
                    ...
                    max_imgs = min(batch.size(0), 48)
                    reconstruction_norm = normalize_to_0_1(rec)
                    reconstruction = mask_background_to_zero(reconstruction_norm, mask)

                    vutils.save_image(
                        torch.cat([real_batch[(max_imgs - 8):max_imgs], rec[(max_imgs - 8):max_imgs],
                                   reconstruction_norm[(max_imgs - 8):max_imgs],
                                   reconstruction[(max_imgs - 8):max_imgs],
                                   real_batch_mask[(max_imgs - 8):max_imgs]], dim=0).data.cpu(),
                ...
                # loss_rec = calc_reconstruction_loss(real_batch*mask, rec*mask, loss_type=recon_loss_type, reduction="mean")
                _, _, loss_rec, _, _ = loss_function(rec, real_batch, mask, real_mu, real_logvar)
                ...
                masked = rec*mask
                masked[mask == 0] = -3.5
                rec_mu, rec_logvar, z_rec, rec_rec = model(masked.detach())
                ...
                loss_rec_rec_e = calc_reconstruction_loss(rec*mask, rec_rec*mask, loss_type=recon_loss_type, reduction='none')
                # _, _, loss_rec_rec_e, _, _ = loss_function(rec, real_batch, mask, real_mu, real_logvar)
                while len(loss_rec_rec_e.shape) > 1:
                    loss_rec_rec_e = loss_rec_rec_e.sum(-1)
                loss_rec_fake_e = calc_reconstruction_loss(fake*mask, rec_fake*mask, loss_type=recon_loss_type, reduction='none')
                ...
                encoder_loss_print.append(lossE.data.cpu().item())
                optimizer_e.zero_grad()
                lossE.backward()
                optimizer_e.step()
                ...
                masked = rec * mask
                masked[mask == 0] = -3.5
                rec_mu, rec_logvar = model.encode(masked)
                ...
                _, _, loss_rec_rec, _, _ = loss_function(rec_rec, rec.detach(), mask, real_mu, real_logvar)

                # loss_fake_rec = calc_reconstruction_loss(fake.detach(), rec_fake, loss_type=recon_loss_type, reduction="mean")
                _, _, loss_fake_rec, _, _ = loss_function(rec_fake, fake.detach(), mask, real_mu, real_logvar)


                lossD_rec_kl = calc_kl(rec_logvar, rec_mu, reduce="mean")
                lossD_fake_kl = calc_kl(fake_logvar, fake_mu, reduce="mean")

                lossD = scale * (loss_rec * beta_rec + (lossD_rec_kl + lossD_fake_kl) * 0.5 * beta_kl
                                 + gamma_r * 0.5 * beta_rec * (loss_rec_rec + loss_fake_rec))

                decoder_loss_print.append(lossD.data.cpu().item())
                optimizer_d.zero_grad()
                lossD.backward()
                optimizer_d.step()
                ...
                batch_elbo_real.append(lossE_real.data.cpu())
                ...
                if cur_iter % test_iter == 0:
                    ...
                    real_batch_mask = mask_background_to_zero(real_batch_norm, mask)
                    max_imgs = min(batch.size(0), 48)
                    reconstruction_norm = normalize_to_0_1(rec_det)
                    reconstruction = mask_background_to_zero(reconstruction_norm, mask)

                    vutils.save_image(
                        torch.cat([real_batch[(max_imgs - 8):max_imgs], rec_det[(max_imgs - 8):max_imgs],
                                  reconstruction_norm[(max_imgs - 8):max_imgs], reconstruction[(max_imgs - 8):max_imgs],
                                   real_batch_mask[(max_imgs - 8):max_imgs]], dim=0).data.cpu(),
                        '{}/image_{}.jpg'.format(fig_dir, cur_iter), nrow=num_row)
        ...
        # writer.add_scalar("LossE/train", sum(encoder_loss_print) / len(encoder_loss_print), epoch)
        # writer.add_scalar("LossE_expelbo_fake/train", sum(batch_exp_elbo_f) / len(batch_exp_elbo_f), epoch)
        # writer.add_scalar("LossE_expelbo_rec/train", sum(batch_exp_elbo_r) / len(batch_exp_elbo_r), epoch)
        # writer.add_scalar("LossE_elbo_real/train", sum(batch_elbo_real) / len(batch_elbo_real), epoch)
        writer.add_scalar("LossE_kldiv_real/train", sum(batch_kls_real) / len(batch_kls_real), epoch)
        # writer.add_scalar("LossD_kldiv_rec/train", sum(batch_kls_rec) / len(batch_kls_rec), epoch)
        # writer.add_scalar("LossD_kldiv_fake/train", sum(batch_kls_fake) / len(batch_kls_fake), epoch)
        # writer.add_scalar("LossD/train", sum(decoder_loss_print) / len(decoder_loss_print), epoch)
        writer.add_scalar("LossD_recon_error/train", sum(batch_rec_errs) / len(batch_rec_errs), epoch)
        writer.add_scalar("Loss_SimpleVAE/train", sum(simple_loss_VAE) / len(simple_loss_VAE), epoch)
        pbar.close()
        ...
        if epoch > num_vae - 1:
            ...
            print(f'rec: {rec_errs[-1]:.3f}, kl: {kls_real[-1]:.3f}') # for simple vae training losses
            # print(
        ...
        if epoch == num_epochs - 1:
            # save models
            # prefix = "16_2_20220/" + dataset + "_Simpple_VAE_beta_rec_" + str(beta_rec) + "/"
            prefix = "24_4_20220/" + dataset + "_Simpple_VAE_beta_rec_" + str(beta_rec) + f"_beta_kl_{beta_kl}/"
            # prefix = "19_4_20220/" + dataset + "_bv3.5_maskedrec" + "_soft_intro" + "_betas_" + str(
            #     beta_kl) + "_" + str(beta_neg) + "_" + str(beta_rec) + f"_seed_{seed}/"
            ...
            # ax.legend()
            # plt.savefig('./' + prefix + 'soft_intro_train_graphs.jpg')
            # with open('./' + prefix + 'soft_intro_train_graphs_data.pickle', 'wb') as fp:
            #     # graph_dict = {"rec_err": rec_errs}
            #     # graph_dict = {"kl_real": kls_real, "rec_err": rec_errs}
...
def domain_adaptaion(dataset='camcan', target='t2', epochs=100, device=torch.device("cpu"), save_interval=1,
    ...
    for epoch in range(epochs):
        ...
        if epoch % save_interval == 0 and epoch > 0:
            save_epoch = (epoch // save_interval) * save_interval
            prefix = "4_5_20220/" + dataset + f"_domain_adaptation_{target}_batch_40_num_patients_" + str(num_patients) + \
                     "_lr_001_dcd_changes/"
            save_checkpoint(model, save_epoch, cur_iter, prefix)
...
def domain_adaptation_last(dataset='camcan', target='t2', train_mode="soft_intro", epochs=200, num_patients=50, seed=-1,
                           pretrained="./10_3_20220/camcan_bv3.5_maskedrec_soft_intro_betas_1.0_256_3000.0/"
                                "model_epoch_51_iter_30345.pth", pretrained_dcd="./4_4_20220/camcan_domain_adaptation_t2_batch_40_num_patients_50_lr_01_dcd_changes/" \
                     "model_epoch_58_iter_45240.pth", device=torch.device("cpu"), z_dim=128, lr_e=2e-4,
                           lr_d=2e-4, save_interval=2, recon_loss_type="l1", beta_kl=1.0, beta_rec=3000.0, beta_neg=256,
                           beta_dcd=0.1, test_iter=1000, num_row=8, gamma_r=1e-8):
    ...
    if dataset == 'camcan':
        ...
        print(f"Sampling training data took: {interval}")
        X_t, Y_t = create_target_samples(brats17_set_path, BRATS_CAMCAN_DEFAULT_TRANSFORM, n=39 * num_patients)

    experiment_dir = './4_5_20220'
    os.makedirs(experiment_dir, exist_ok=True)

    fig_dir = experiment_dir + f'/{train_mode}/Figures_masked_reconlosses_da_step3_' + dataset + \
              f"_beta_rec_{beta_rec}_beta_neg_{beta_neg}_beta_kl_{beta_kl}_beta_dcd_{beta_dcd}_{target}_{num_patients}" \
              f"_negative_dcd_loss"
    os.makedirs(fig_dir, exist_ok=True)

    loss_fn_dcd = torch.nn.CrossEntropyLoss()

    # Initialize Discriminator trained from STEP 2
    pretrained_dcd = pretrained_dcd
    discriminator = DCD(input_features=256).to(device)
    if pretrained_dcd is not None:
        load_model(discriminator, pretrained_dcd, device)
    discriminator.train()

    # Initialize S-Intro model trained from STEP 1
    model = BaurModel().to(device)
    if pretrained is not None:
        load_model(model, pretrained, device)
    model.train()
    ...
    optimizer_e = optim.Adam(model.encoder.parameters(), lr=lr_e)
    optimizer_d = optim.Adam(model.decoder.parameters(), lr=lr_d)
    optimizer_dcd = torch.optim.Adam(discriminator.parameters(), lr=0.001)
    ...
    for epoch in range(epochs):
        ...
        if epoch % save_interval == 0 and epoch > 0:
            save_epoch = (epoch // save_interval) * save_interval
            prefix = f"4_5_20220/{train_mode}/" + dataset + f"_domain_adaptation_{target}" + "_num_patients_" + str(num_patients) + \
                     "_beta_dcd_" + str(beta_dcd) + "_negative_dcd_loss/"
            save_checkpoint(model, save_epoch, cur_iter, prefix)

        groups, groups_y = create_groups(X_s, Y_s, X_t, Y_t, seed=300 + epoch)
        G1, G2, G3, G4 = groups
        groups_2 = [G2, G4]
        ...
        n_iters_dcd = 4 * len(G2)  # as we are making it for 4 groups
        ...
        for index in range(n_iters):
            ...
            # YOU CAN ADD AND ITS LOSS IF YOU HAVE HEADS FOR AUXILIARY TASKS TO ADD TO THE DOMAIN CONFUSION WITH THE LOSS
            # y1, y2 = groups_y_2[ground_truth][index_list[index] - len(G2) * ground_truth]
            # y1=torch.LongTensor([y1.item()])
            # y2=torch.LongTensor([y2.item()])
            dcd_label = 0 if ground_truth in (0, 1) else 2
            X1.append(x1)
            X2.append(x2)
            dcd_labels.append(dcd_label)

        size = 20  # len(X1) // len(pbar) + 1  # bc the division will give the ceil
        X1_split, X2_split, dcd_labels_split = [], [], []
        for i in range(0, len(X1), size):
            X1_split.append(X1[i:(i + size)])
            X2_split.append(X2[i:(i + size)])
            dcd_labels_split.append(dcd_labels[i:(i + size)])
        ...
        for index in range(n_iters_dcd):
            ...
            x1_dcd, x2_dcd = groups[ground_truth][index_list_dcd[index] - len(groups[1]) * ground_truth]
            X1_dcd.append(x1_dcd)
            X2_dcd.append(x2_dcd)
            ground_truths.append(ground_truth)

        mini_batch_size_dcd = 40  # data contains G1,G2,G3,G4 so use 40 as mini_batch
        X1_dcd_split, X2_dcd_split, ground_truths_split = [], [], []
        for i in range(0, len(X1_dcd), mini_batch_size_dcd):
            X1_dcd_split.append(X1_dcd[i:(i + size)])
            X2_dcd_split.append(X2_dcd[i:(i + size)])
            ground_truths_split.append(ground_truths[i:(i + size)])
        ...
        for batch in pbar:
            if train_mode == "simple":
                ...
                # for the dcd data
                if batch_iter >= len(dcd_labels_split):
                    batch_iter = 0
                dcd_labels_chosen = torch.LongTensor(dcd_labels_split[batch_iter]).to(device)
                dcd_mu1, dcd_logvar1 = model.encode(torch.stack(X1_split[batch_iter]).to(device))
                dcd_z1 = reparameterize(dcd_mu1, dcd_logvar1)
                dcd_mu2, dcd_logvar2 = model.encode(torch.stack(X2_split[batch_iter]).to(device))
                dcd_z2 = reparameterize(dcd_mu2, dcd_logvar2)
                dcd_z = torch.cat([dcd_z1, dcd_z2], 1)
                dcd_pred = discriminator(dcd_z)
                loss_dcd = loss_fn_dcd(dcd_pred, dcd_labels_chosen)

                # loss = loss_rec
                loss = loss_rec + loss_kl - beta_dcd * loss_dcd
                ...
                batch_kls_real.append(loss_kl.data.cpu().item())
                batch_Loss_DCD.append(loss_dcd.cpu().item())

                # =========== Update D, DCD, E frozen ================
                for param in model.encoder.parameters():
                    param.requires_grad = False
                for param in model.decoder.parameters():
                    param.requires_grad = True
                ...
                # for the dcd data
                if batch_iter >= len(ground_truths_split):
                    batch_iter = 0
                dcd_labels_chosen = torch.LongTensor(ground_truths_split[batch_iter]).to(device)
                dcd_mu1, dcd_logvar1 = model.encode(torch.stack(X1_dcd_split[batch_iter]).to(device))
                dcd_z1 = reparameterize(dcd_mu1, dcd_logvar1)
                dcd_mu2, dcd_logvar2 = model.encode(torch.stack(X2_dcd_split[batch_iter]).to(device))
                dcd_z2 = reparameterize(dcd_mu2, dcd_logvar2)
                dcd_z = torch.cat([dcd_z1, dcd_z2], 1)
                dcd_pred = discriminator(dcd_z)
                loss_dcd_train = loss_fn_dcd(dcd_pred, dcd_labels_chosen)

                optimizer_dcd.zero_grad()
                loss_dcd_train.backward()
                optimizer_dcd.step()
                training_loss_dcd.append(loss_dcd_train)

                pbar.set_description_str('epoch #{}'.format(epoch))
                pbar.set_postfix(r_loss=loss_rec.data.cpu().item(), kl=loss_kl.data.cpu().item(),
                                 dcd_train=loss_dcd_train.data.cpu().item())
                ...
                loss_rec_rec_e = calc_reconstruction_loss(rec * mask, rec_rec * mask, loss_type=recon_loss_type,
                                                          reduction='none')
                # _, _, loss_rec_rec_e, _, _ = loss_function(rec, real_batch, mask, real_mu, real_logvar)
                while len(loss_rec_rec_e.shape) > 1:
                    loss_rec_rec_e = loss_rec_rec_e.sum(-1)
                loss_rec_fake_e = calc_reconstruction_loss(fake * mask, rec_fake * mask, loss_type=recon_loss_type,
                                                           reduction='none')
                while len(loss_rec_fake_e.shape) > 1:
                    loss_rec_fake_e = loss_rec_fake_e.sum(-1)
                ...
                lossE = lossE_real + lossE_fake - beta_dcd * loss_dcd
                encoder_loss_print.append(lossE.data.cpu().item())
                optimizer_e.zero_grad()
                lossE.backward()
                optimizer_e.step()
                ...
                dcd_z = torch.cat([dcd_z1, dcd_z2], 1)
                dcd_pred = discriminator(dcd_z)
                loss_dcd_training = loss_fn_dcd(dcd_pred, dcd_labels_chosen)

                optimizer_dcd.zero_grad()
                loss_dcd_training.backward()
                optimizer_dcd.step()
                training_loss_dcd.append(loss_dcd_training)
        ...
        writer.add_scalar("LossD/train", sum(decoder_loss_print) / len(decoder_loss_print), epoch)
        writer.add_scalar("LossD_recon_error/train", sum(batch_rec_errs) / len(batch_rec_errs), epoch)
        writer.add_scalar("Loss_DCD/frozen", sum(batch_Loss_DCD) / len(batch_Loss_DCD), epoch)
        writer.add_scalar("Loss_DCD/training", sum(training_loss_dcd) / len(training_loss_dcd), epoch)
        # writer.add_scalar("Loss_simpleVAE/train", sum(simple_loss_VAE) / len(simple_loss_VAE), epoch)
        pbar.close()
    writer.close()
...
if __name__ == '__main__':
    ...
    # pretrained = "./9_4_20220/camcan_domain_adaptation_t2_batch_40_num_patients_1_lr_001_dcd_changes/model_epoch_298_iter_4470.pth"
    # pretrained = "./9_4_20220/camcan_domain_adaptation_t2hm_batch_40_num_patients_50_lr_001_dcd_changes/model_epoch_58_iter_45240.pth"
    pretrained_s_intro = "./10_3_20220/camcan_bv3.5_maskedrec_soft_intro_betas_1.0_256_3000.0/model_epoch_51_iter_30345.pth"
    # for simple da:
    # pretrained_dcd_simple = "./19_4_20220/camcan_domain_adaptation_t2_batch_40_num_patients_50_lr_001_dcd_changes/model_epoch_298_iter_232440.pth"model_epoch_290_iter_113100.pth
    pretrained_simple = "./camcan_bv3.5_maskedrec_simpleVAE_betas_1.0_256_100.0/model_epoch_149_iter_89250.pth"
    # print(pretrained)
    print(device)
    print("betas: ", beta_kl, beta_neg, beta_rec, beta_dcd)
    try:
        # domain_adaptaion(dataset='camcan', target='t2', epochs=300, device=device, save_interval=2,
        #                  pretrained=pretrained_s_intro,
        #                  seed=0, num_patients=5)
        domain_adaptation_last(dataset='camcan', target='t2', train_mode="soft_intro", epochs=100, num_patients=5, seed=-1,
                               pretrained=pretrained_s_intro,
                               pretrained_dcd="./4_5_20220/camcan_domain_adaptation_t2_batch_40_num_patients_5_lr_001_dcd_changes/model_epoch_290_iter_22620.pth",
                               device=device, z_dim=128, lr_e=1e-4, lr_d=1e-4, save_interval=2, recon_loss_type="l1",
                               beta_kl=beta_kl, beta_rec=beta_rec, beta_neg=beta_neg, beta_dcd=beta_dcd, test_iter=500,
                               num_row=8, gamma_r=1e-8)
        # train_soft_intro_vae(dataset="camcan", z_dim=128, batch_size=128, num_workers=0, num_epochs=85,
        #                      num_vae=85, beta_kl=beta_kl, beta_neg=beta_neg, beta_rec=beta_rec,
        #                      device=device, save_interval=1, start_epoch=0, lr_e=1e-4, lr_d=1e-4,
        #                      pretrained=None, test_iter=1000, with_fid=False, seed=-1)
        # changed  lr to e-4 from 2e-4 for camcan and epochs to 80 and save interval
    except SystemError:
